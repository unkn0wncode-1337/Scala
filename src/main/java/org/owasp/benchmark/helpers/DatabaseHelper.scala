...
class DatabaseHelper  {
    ...
    static {
        ...
        return conn
            ...
            var numberOfColumns: Int = rsmd.getColumnCount()
            ...
            while (rs.next()) {
                for (i <- 1 to numberOfColumns) {
                    ...
                    var columnValue: String = rs.getString(i)
...
object DatabaseHelper {
    ...
    def getSqlStatement(): java.sql.Statement = {
        if (conn == None) {
            getSqlConnection()
    }
    def reset(): Unit = {
    ...
    def getSqlConnection(): java.sql.Connection = {
        if (conn == None) {
            try {
                var ctx: InitialContext = new InitialContext()
                var datasource: DataSource = (DataSource) ctx.lookup("java:comp/env/jdbc/BenchmarkDB")
    ...
    def executeSQLCommand(sql: String): Unit = {
        var stmt: Statement = getSqlStatement()
        stmt.executeUpdate(sql)
    }
    def outputUpdateComplete(sql: String, response: HttpServletResponse): Unit = {
    ...
    def outputUpdateComplete(sql: String, resp: List[XMLMessage]): Unit = {
            throws java.sql.SQLException, IOException {
    ...
    def printResults(): Unit = {
        ...
        try {
            var rs: ResultSet = statement.getResultSet()
            if (rs == None) {
                out.write(
    ...
    def printResults(statement: java.sql.Statement, sql: String, resp: List[XMLMessage]): Unit = {
            throws java.sql.SQLException, IOException {
    ...
    def printResults(rs: java.sql.ResultSet, sql: String, response: HttpServletResponse): Unit = {
            throws java.sql.SQLException, IOException {
    ...
    def printResults(rs: java.sql.ResultSet, sql: String, resp: List[XMLMessage]): Unit = {
            throws java.sql.SQLException, IOException {
    ...
    def printResults(query: String, counts: Array[int], response: HttpServletResponse): Unit = {
            throws IOException {
        var out: PrintWriter = response.getWriter()
        out.write("<!DOCTYPE html>\n<html>\n<body>\n<p>")
    ...
    def printResults(query: String, counts: Array[int], resp: List[XMLMessage]): Unit = {
            throws IOException {
    ...
    def printColTypes(rsmd: ResultSetMetaData, out: PrintWriter): Unit = {
            throws java.sql.SQLException {
        var columns: Int = rsmd.getColumnCount()
        for (i <- 1 to columns) {
            var jdbcType: Int = rsmd.getColumnType(i)
            var name: String = rsmd.getColumnTypeName(i)
            out.write(
                    "Column "
                            + i
                            + " is JDBC type "
