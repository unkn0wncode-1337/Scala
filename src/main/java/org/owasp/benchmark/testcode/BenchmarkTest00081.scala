...
class BenchmarkTest00081 extends HttpServlet {
    ...
    def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
        ...
        var stuff: Double = new java.util.Random().nextGaussian()
        var rememberMeKey: String = Double.toString(stuff).substring(2); // Trim off the 0. at the front.

        var user: String = "Gayle"
