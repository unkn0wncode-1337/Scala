...
class BenchmarkTest00095 extends HttpServlet {
    ...
    def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
        ...
        try {
            var stuff: Double = java.security.SecureRandom.getInstance("SHA1PRNG").nextGaussian()
            String rememberMeKey =
                    Double.toString(stuff).substring(2); // Trim off the 0. at the front.

            var user: String = "SafeGayle"
