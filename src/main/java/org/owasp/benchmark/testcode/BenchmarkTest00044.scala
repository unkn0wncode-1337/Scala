...
class BenchmarkTest00044 extends HttpServlet {
    ...
    def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
        ...
        var values: Array[String] = request.getParameterValues("BenchmarkTest00044")
        String param
        if (values != None && values.length > 0) param = values[0]
        var param: else = ""
        ...
        try {
            ...
            var filter: String = "(&(objectclass=person)(uid=" + param + "))"
